...
class TwitterRequest extends FormRequest
{
        ...
        $rules = [
            'title' => [
                ...
                function ($attribute, $value, $fail) {
                    $query = Twitter::where('title', $value)
                        ->where('content_date', $this->content_date);

                    // Exclude saat Update
                    if ($this->twitter) {
                        $query->where('id', '!=', $this->twitter->id);
                    }

                    if ($query->exists()) {
                        $fail('Konten Twitter dengan judul dan tanggal ini sudah ada!');
                    }
                },
            ],
            'link' => ['nullable', 'url'],
    ...
    // ATTRIBUT-ATRIBUT DI DALAM TABEL TWITTER
