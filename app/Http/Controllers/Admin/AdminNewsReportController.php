...
class AdminNewsReportController extends Controller
{
    /* ==================================== */
    /* =========== REKAP BERITA =========== */
    /* ==================================== */

    /* REKAP BULAN (REPORT TABLE) */
    public function exportMonthlyReport(string $year, string $month)
    {
        ...
        // Compress cover_image (Screenshot) sebelum masuk ke PDF
        foreach ($news as $item) {
            if ($item->cover_image) {
                ...
                if (file_exists($path)) {
                    try {
                        // Resize max width (800px) & Compress ke format JPG berkualitas 70%
                        $image = $manager->read($path)
                            ->scale(width: 800)
                            ->toJpeg(70);
                        
                        // Simpan file ke folder cache
                        ...
                        // Simpan file ke dalamm folder path baru
        ...
        $pdf = Pdf::loadView('admin.news.format_pdf.monthly', compact('news', 'month', 'year', 'monthName'))
            ->setPaper('A4', 'potrait')
            ->setOption('dpi', 96)
    ...
    public function exportYearlyReportByYear(Request $request)
    {
        ...
        // Inisialisasi ImageManger (menggunakan GD Driver)
        $manager = new ImageManager(new Driver());
        ...
        $pdf = Pdf::loadView('admin.news.format_pdf.yearly', [
            'news' => $news,
            'year' => $year,
        ])
            ->setPaper('A4', 'potrait')
    ...
    public function exportMonthlyReportByMonth(Request $request)
    {
        ...
        if (!$monthInput) {
            return back()->with('error', 'Bulan & Filter harus dipilih.');
        }
        ...
        // KOMPRES IMAGES SEBELUM MASUK KE PDF
        foreach ($news as $item) {
            if ($item->cover_image) {
                $path = storage_path("app/public/{$item->cover_image}");
        ...
        // GENERATE PDF
        $pdf = Pdf::loadView('admin.news.format_pdf.monthly', [
            'news' => $news,
            'year' => $year,
            'month' => $month,
            'monthName' => $monthName,
    ...
    public function exportMonthlyReportByOfficeOrSumberForm(Request $request)
    {
        ...
        $pdf = Pdf::loadView('admin.news.format_pdf.monthly_filter', [
            'news' => $news,
            'year' => $year,
            'month' => $month,
            'monthName' => $monthName,
    ...
    public function exportYearlyReportByOfficeOrSumberForm(Request $request)
    {
            ...
            if ($item->cover_image) {
                ...
                if (file_exists($path)) {
                    try {
                        ...
                        // Simpan ke folder path baru
                        $item->compressed_image = "pdf-cache-{$item->id}.jpg";
                    } catch (\Exception $e) {
                        // Fallback -> pakai original
                        $item->compressed_image = $item->cover_image;
                    }
        ...
        $pdf = Pdf::loadView('admin.news.format_pdf.yearly_filter', [
            'news' => $news,
            'year' => $year,
            'filterType' => $filterType,
            'filterValue' => $filterValue,
